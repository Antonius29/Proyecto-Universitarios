...
function mostrarError(result) {
  ...
  switch (result.tipo_error) {
    ...
    case "CONTRASEÑA_INCORRECTA":
      ...
      const intentosRestantes = result.intentos_restantes !== undefined ? result.intentos_restantes : 'N/A';
      errorText.innerHTML = `<strong>Intentos restantes: ${intentosRestantes}</strong><br>${result.detalles}`;
      break
