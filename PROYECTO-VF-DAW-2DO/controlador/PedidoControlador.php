...
class PedidoControlador {
    ...
    public function guardar() {
        ...
        // Validaciones
        if (empty($clienteId) || empty($fechaPedido) || empty($metodoPago) || empty($direccionEnvio) || $total <= 0) {
    ...
    public function actualizar() {
        ...
        // Validaciones
        if ($id <= 0 || empty($clienteId) || empty($fechaPedido) || empty($metodoPago) || empty($direccionEnvio) || $total <= 0) {
            $_SESSION['error'] = 'Por favor complete todos los campos obligatorios';
